...
企業名・サービス名・アプリ名のリストを渡すだけで、ロゴ画像の取得・背景透過・トリミングを自動化し、**PowerPoint にロゴが貼られた状態**まで最短距離で到達させる。単発の個人ツールではなく、常時稼働のサーバーを持たないチームでも、GitHub のアカウントさえあれば誰でも取得を実行でき、結果が共有ストックとして蓄積・検索・再利用できる状態を最終形とする。
...
- remove.bg が受け付けない形式(GIF等)は、送る前に PNG へ変換する。公式サイトのロゴがGIFで置かれている場合に「やり直し」が永久に失敗し続けるのを防ぐ `[unit test]`
...
- ロゴの「選択」チェックは画面全体で1つの状態にする。同じロゴが複数のセクションに出るため、片方でチェックしたのにもう片方のボタンが効かない、という取り違えを起こさない `[qcheck]`(JS、pytest対象外)
- 実行中は取得・やり直し・削除のボタンをすべて無効化し、進捗テキストを全セクションに表示する。押した場所から反応が見えないことによる二重実行を防ぐ `[qcheck]`(JS、pytest対象外)
